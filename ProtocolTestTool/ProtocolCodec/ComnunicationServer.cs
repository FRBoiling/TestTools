...
namespace LibProtocolCodec
{
    public class ComnunicationServer : AbstractTcpServer
    {
        public ProtocolPacket1 packetHandler = new ProtocolPacket1();

        protected Queue<Packet> m_msgQueue = new Queue<Packet>();
        protected Queue<Packet> m_deal_msgQueue = new Queue<Packet>();


        public ComnunicationServer( )
        {
        }
        ...
        public override void AcceptComplete(bool ret)
        {
            base.AcceptComplete(ret);
            bAccepted = ret;
            if (bAccepted)
            {
                Console.WriteLine("AcceptComplete success!(loacl:{0})", Tcp.WorkSocket.LocalEndPoint.ToString());
                AccpetCallback(bAccepted);
            }
        }

        public delegate void Accpeted(bool bAccpeted);
        public Accpeted AccpetCallback;
