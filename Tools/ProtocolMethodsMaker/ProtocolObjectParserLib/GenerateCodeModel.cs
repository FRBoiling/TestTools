...
namespace ProtocolObjectParserLib
{
    public class GenerateCodeModel
    {
        ...
        private static string ClassBodyStr(Dictionary<string, string> keys)
        {
            ...
            {
                if (item.Value.Contains("Message") && !item.Value.Contains("+"))
                {
        ...
        private static string DeserializeMsgFunc(string key)
        {
            ...
            sb.Append("{");
            sb.Append(Environment.NewLine);
            tempString = string.Format("return  ProtoBuf.Serializer.Deserialize<{0}>(stream);", key);
            sb.Append(tempString);
            sb.Append(Environment.NewLine);
            sb.Append("}");
            string code = sb.ToString();
        ...
        private static string BindMsgIdFunc()
        {
            ...
            sb.Append("{");
            sb.Append(Environment.NewLine);
            sb.Append("Message.Gate.Protocol.GateC.Api.GenerateId();");
            sb.Append(Environment.NewLine);
            sb.Append("Message.Client.Protocol.CGate.Api.GenerateId();");
            sb.Append(Environment.NewLine);
            sb.Append("Message.Barrack.Protocol.BarrackC.BarrackCIdGenerator.GenerateId();");
            sb.Append(Environment.NewLine);
            sb.Append("Message.Client.Protocol.CBarrack.Api.GenerateId();");
            sb.Append(Environment.NewLine);
            sb.Append("}");
            string code = sb.ToString();
            return code;
